...
- ✅ **90d 历史 backfill 已启动**（2026-02-02 ~ 现在）—— 进行中，预计 ~30h 跑完
- ✅ **PancakeSwap V4 CL** 池监控已上线（dex=`pancakeswap-v4-cl`）
- ✅ **PancakeSwap V2 WBNB/USDT** 池监控（仅记录 BNB 历史价 → `bnb_price_history` 表）
- ✅ **swaps 唯一索引** `uq_swaps_dedup(tx_hash, pool_address, amount0, amount1)` 防 stream+backfill 双写重复
- ✅ **PG 调优**：shared_buffers 128MB → 2GB，从 wait IO/DataFileRead 解放出来（CPU 310% → 15%）
...
# 跑完后从 swaps 重建 buckets（覆盖累加偏差）
ssh root@107.175.35.109
cd /opt/bsc-binance-radar

# 30d backfill ≈ 6.7h（8 worker）
docker compose run --rm --no-deps radar bun scripts/backfill.ts 30d 8

# 90d backfill ≈ 30h（90d 前数据密度高 + cache miss 多，比 30d 慢得多）
docker compose run --rm --no-deps radar bun scripts/backfill.ts 90d 8

# 长任务用 tmux：
tmux new -d -s backfill "docker compose run --rm --no-deps radar bun scripts/backfill.ts 90d 8 > backups/backfill.log 2>&1"
...
- 安全设置：`{wait:10, batchSize:2}` 最多合 2 个 RPC，response ≤ 10MB
