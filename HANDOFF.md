...
- 🔄 **90d 历史 backfill 优化版正在跑**（2026-02-02 ~ 现在），双 RPC shard，**ETA ~17h（vs 优化前 60h，3.5x）**
  - tmux `bf-self`（自建节点 151.123.172.62:81）：跑 block 78852395 → 86093195（前 42% 范围，~38d 旧数据）
  - tmux `bf-nr`（NodeReal 公网）：跑 block 86093195 → 96092395（后 58% 范围，~52d 新数据）
  - 两 shard 都设了 `BF_SKIP_REBUILD=1`，**跑完后必须手动跑 `scripts/rebuild-buckets.ts 90d`** 重建 buckets
- ✅ **PancakeSwap V4 CL** 池监控已上线（dex=`pancakeswap-v4-cl`）
- ✅ **PancakeSwap V2 WBNB/USDT** 池监控（仅记录 BNB 历史价 → `bnb_price_history` 表）
- ✅ **swaps 唯一索引** `uq_swaps_dedup(tx_hash, pool_address, amount0, amount1)` 防 stream+backfill/双 shard 三方重复
- ✅ **PG 调优**：shared_buffers 128MB → 2GB（早期），加 5 项 backfill 性能优化（见下文 § 16）
...
| PG | `radar-pg` 容器，宿主 `127.0.0.1:5434`，密码在 `.env`；shared_buffers=2GB |
...
| `swaps` | 原始 swap（**永久保留**） | ~150M（90d backfill 完成后估计） |
...
| `bnb_price_history` | BNB/USD 历史价（来自 PancakeV2 WBNB/USDT 池）**新** | ~10M+（每秒级） |

**唯一索引**：
- `swaps.uq_swaps_dedup (tx_hash, pool_address, amount0, amount1)` —— stream+backfill 双写靠它去重
...
PG_POOL_MAX=40                         # 新：pg pool 连接数（默认 10 不够 8 worker × 4 INSERT）
...
**双 RPC sharding（90d 用，~17h，比单节点 60h 快 3.5x）**
```bash
# 1. 算 block 范围
...
SELF_END=$((START + 17240000*42/100))   # self 拿前 42%（旧数据，自建快）
# nr 拿后 58%（新数据，NodeReal）

# 2. 启 self shard（自建节点 8 worker，跑前段）
tmux new -d -s bf-self "docker compose run --rm --no-deps \
  -e BF_FROM_BLOCK=$START -e BF_TO_BLOCK=$SELF_END \
  -e BF_SHARD_LABEL=self -e BF_SKIP_REBUILD=1 \
  radar bun scripts/backfill.ts 90d > backups/bf-self.log 2>&1"

# 3. 启 nr shard（NodeReal 8 worker，跑后段）。⚠️ NodeReal concurrency 不要超 8，超会撞限。
tmux new -d -s bf-nr "docker compose run --rm --no-deps \
  -e BF_FROM_BLOCK=$SELF_END -e BF_TO_BLOCK=$LATEST \
  -e BF_SHARD_LABEL=nr -e BF_SKIP_REBUILD=1 \
  -e BF_RPC_URL=https://bsc-mainnet.nodereal.io/v1/b13fcff9775e4d1bb28a0735292a1819 \
  radar bun scripts/backfill.ts 90d > backups/bf-nr.log 2>&1"

# 4. 等两 shard 都跑完后，统一重建 buckets（必须，因为两 shard 都设了 BF_SKIP_REBUILD）
...
### 16. backfill 性能优化轮（2026-05-03，60h → 17h, 3.5x）
...
- 多 shard 都设 `BF_SKIP_REBUILD=1`，跑完后跑 `scripts/rebuild-buckets.ts 90d` 统一重建
...
剩下还可能压榨的（未做）：
- 删 swaps 冗余索引（`idx_swaps_chain_dex_time` 跟 `idx_swaps_time_chain_valid` 部分重叠）—— 风险中等
- flush 异步化（worker 不等 PG）—— PG CPU 已 240%+，再压 PG 可能崩
